...
class StarRating(QObject):
    ...
    def __init__(self,parent=None):
        ...
        rotation=.33
        points=[]
        # m, n control the inner and outer radius of the star
        # if n is larger relative to m --> pentagram
        # if m is larger relative to n --> very spokey star
        m = 1.5
        n = 1
        for i in reversed(list(range(0,11))):
            w = 2*(i+rotation)*math.pi/10
            # set every other point to the max or min radius
            r = (m*(i%2)+n)/(m+n)
            # if you want the star centered at a point, remove the +1
...
class StarEdit(QWidget):
    ...
    def mouseMoveEvent(self,event):
        new_value = self.starAtPosition(event.x())
        if new_value != self._value:
            self._value = new_value
            self.starrating.setValue(self._value)
            self.update()
    ...
    def starAtPosition(self,x):
        return max(0,min(5,int(6 * x / self.width())));
...
class TextDelegate(StyledItemDelegate):
    """ Edit text in a cell for a table

    This is a simple example of a text editor for the table.
    The default delegate provided by Qt will erase the existing content
    when editing begins, this implementation instead preserves the existing
    text.

    This could be extended to provide input validation, and editing
    of non-text values. e.g. a float value whcih is converted to and
    from a string.
    """
    def __init__(self,parent=None):
...
class TableColumn(QObject):
    ...
    def displayName(self,width=0):
        ...
        if self._name != self.__cached_name:
            ...
            self.__cached_name_length = QFontMetrics(QApplication.font()).width("   "+self._name+"   ")
    ...
    def data(self,tabledata,row):
        return tabledata[row][self._key]
...
class TableModel(QAbstractTableModel):
    """docstring for TableModel"""

    OriginalDataRole = Qt.UserRole + 1
    ...
    def columnIndex(self,key):
        ...
        not a 1-to-1 mapping,not invertable, can't be implemented"""
    ...
    def data(self, index, role):
        ...
        if role == Qt.DisplayRole:
            ...
            for rule in self._backgroundRules:
                ...
                if result is not None:
                    return result
        elif role == TableModel.OriginalDataRole:
...
class SortProxyModel(QSortFilterProxyModel):
    ...
    def headerData(self, section, orientation, role = Qt.DisplayRole):
        ...
        if orientation == Qt.Vertical and self._enable_stable_index:
            return section + 1
...
class AbstractTableView(QTableView):
    ...
    def __init__(self, parent):
        ...
        # set this to true to enable reorderable columns
        self.horizontalHeader().setSectionsMovable(False);

        # force row size to be uniform
    ...
    def mousePressEvent(self,event):
        ...
        # TODO: should I add separate handlers for these events:
    ...
    def setSortingEnabled(self, bEnabled):
        ...
        if bEnabled:
            ...
            super(AbstractTableView,self).setModel(self._baseModel)
    ...
    def getSelectedRows(self):
        ...
        model = self.selectionModel();
        return [i.row() for i in model.selectedRows()]
    ...
    def getSelectedRowData(self):
        ...
        # qt does not provide an easy way to recover the original table
        # data that has been selected, when tables can be sorted by row.
        # the custom OriginalDataRole allows for iterating over the set
        # of selected cells (all row/columns) and returning unique row info.
        ...
        for index in self.selectionModel().selection().indexes():
            ...
            if row in visited:
                continue
            visited.add(row)
            yield index.data(TableModel.OriginalDataRole)
...
class _DemoMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.list.setHeaderText("Header")
